...
export default {
  ...
  created() {
      ...
      if (value !== undefined) {
        if (prop === "renderHeader") {
          if (type === "selection" && column[prop]) {
            console.warn(
              "[Element Warn][TableColumn]Selection column doesn't allow to set render-header function."
            );
          } else {
    ...
    column.renderCell = function(h, data) {
      ...
      const children = [_self.renderTreeCell(data), renderCell(h, data)];
      ...
      if (data.treeNode.hasChildren) {
        ele.push(
          <div
            class={[
              "el-table__expand-icon",
              data.treeNode.expanded ? "el-table__expand-icon--expanded" : ""
            ]}
  ...
  mounted() {
    ...
    if (this.$scopedSlots.header) {
      if (this.type === "selection") {
        console.warn(
          "[Element Warn][TableColumn]Selection column doesn't allow to set scoped-slot header."
        );
      } else {
