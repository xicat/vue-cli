...
export default {
  ...
  props:{
  	...
  	cellStyle: {
		...
		summaryMethod: {
  		type: Function,
  		default: function() { //Function({ columns, data })
  			return null
  		}					
		},
  ...
  methods:{
  	tdFixedHandle(){
  		...
  		this.$el && this.$el.querySelectorAll('td,th').forEach((item) => {
  			if(!item.children[0]){
  				return true
  			}
  			console.log(item.clientWidth,item.children[0].scrollWidth,item.children[0].innerText)
  			if(item.clientWidth < item.children[0].scrollWidth ){
  				item.title = item.children[0].innerText
  			}else{
  				item.title = ''
  			}
  		})
  	...
  	select(selection, row) {//当用户手动勾选数据行的 Checkbox 时触发的事件
		...
		rowClick(row, event, column) {//当某一行被点击时会触发该事件
			this.$emit('row-click',row, event, column);
		},
		rowContextmenu(row, event) {//当某一行被鼠标右键点击时会触发该事件
			this.$emit('row-contextmenu',row, event);
		},
		rowDblclick(row, event) {//当某一行被双击时会触发该事件
		...
		clearSelection(selection) {//用于多选表格，清空用户的选择，当使用 reserve-selection 功能的时候，可能会需要使用此方法
		...
		toggleRowExpansion(row, expanded) {//用于可展开表格，切换某一行的展开状态，如果使用了第二个参数，则是设置这一行展开与否（expanded 为 true 则展开）
